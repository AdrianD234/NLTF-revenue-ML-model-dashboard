...
def main() -> int:
    ...
    def check_full_sample_gain_chart() -> tuple[bool, str]:
        ...
        if pd.isna(paired_gain) or paired_gain <= 0:
            return fail("Light RUC paired paper-grid gain is not recorded as positive in the v6 evidence pack.")
        if pd.isna(full_gain) or full_gain <= 0:
            return fail("Light RUC paper-style quarterly gain is not recorded as positive in the v6 evidence pack.")
        if pd.isna(annual_gain) or annual_gain <= 0:
            return fail("Light RUC paper-style annual gain is not recorded as positive in the v6 evidence pack.")
