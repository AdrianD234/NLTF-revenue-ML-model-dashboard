...
| 101. Every main chart has a source table. | PASS | 16 chart source tables exist. |
| 102. Chart source tables include the required source contract columns. | PASS | All chart source tables include the core source contract columns. |
| 103. Overview finalist accuracy source reconciles to current Parquet. | PASS | Finalist accuracy source table reconciles to current Parquet values. |
| 104. Ensemble composition source uses Parquet component weights. | PASS | Current finalist component weights match ensemble_components_json. |
| 105. Stress source coalesces aliases and preserves Heavy RUC gaps. | PASS | Stress aliases coalesce and Heavy RUC missing windows remain explicit gaps. |
| 106. Schiff gain chart is labelled full-sample when showing full-sample gains. | PASS | Gain chart is labelled full-sample, not paired. |
| 107. Light RUC paired common-grid weakness is preserved. | PASS | Light RUC paired common-grid gain is negative while full-sample gain is positive and labelled. |
| 108. Scenario decision labels separate full-sample gains and paired win rate. | PASS | Decision summary uses explicit full-sample and paired labels. |
| 109. Horizon chart sources include all streams and scenarios. | PASS | Scenario and Schiff horizon chart sources include all streams and both benchmark scenarios. |
| 110. ACF chart source table exists and documents residual source. | PASS | ACF chart source table exists and documents residual source. |
| 111. Calibration R2 label matches its source field. | PASS | R2 KPI label matches calibration/MZ source semantics. |
| 112. Residual-vs-fitted axis units are not misleading. | PASS | Residual vs fitted axis uses native-unit wording. |
| 113. Candidate count label is precise. | PASS | Candidate count label identifies plotted/default rows. |
| 114. Chart source validation report passed. | PASS | Chart source validation report passed. |
| 115. Semantic label validation report passed. | PASS | Semantic label validation report passed. |
| 116. Visual conformance validation report passed. | PASS | Visual conformance validation report passed. |
| 117. Final screenshots exist for all four pages. | PASS | All final Playwright and page screenshots exist. |
| 118. Screenshot matrix marks all pages PASS. | PASS | Target-vs-current matrix marks all four pages PASS. |
| 119. Filter and hover evidence is present. | PASS | Hover and filter review artifacts contain pass evidence. |
| 120. Existing 100-gate validation has zero failures and backlog is closed. | PASS | Existing visual/data gate suite reports at least 100 passed and zero failed. BUG_BACKLOG.md has no unchecked items. |
