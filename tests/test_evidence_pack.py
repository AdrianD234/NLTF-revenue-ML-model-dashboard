...
def test_default_app_uses_evidence_pack_not_legacy_or_mini(evidence_pack) -> None:
    ...
    assert evidence_pack.manifest["schema_version"] == "dashboard_evidence_pack_v7_vnext_finalists"
...
def test_full_sample_vs_paired_semantics_are_enforced(evidence_pack) -> None:
    ...
    assert full_sample == pytest.approx(3.158190, abs=0.001)
    assert annual == pytest.approx(1.428227, abs=0.001)
