...
def test_candidate_count_label_matches_count_source(parquet_dashboard: LoadedRun) -> None:
    ...
    assert context["label"] == "400 plotted candidates from 400 curated rows"
    note = overview_frontier_note(data["summary"], context)
    assert note.startswith("Frontier read: Balanced all-stream frontier view")
    assert "excluded from governance scoring" in note
    assert "400 plotted candidates from 400 curated rows" in note
    assert "PED 132 frontier rows" in note
    assert "Light RUC 136 frontier rows" in note
    assert "Heavy RUC 132 frontier rows" in note
