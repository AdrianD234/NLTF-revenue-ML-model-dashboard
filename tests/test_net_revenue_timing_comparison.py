...
def test_forty_four_path_net_revenue_matrix_is_complete_unique_and_registry_driven(
    timing_materialization,
) -> None:
    comparison = timing_materialization["comparison"]
    assert isinstance(comparison, pd.DataFrame)
    # 44 paths x 5 fiscal years x 3 series.
    assert len(comparison) == 660
    ...
    assert comparison["path_order"].drop_duplicates().tolist() == list(range(44))
