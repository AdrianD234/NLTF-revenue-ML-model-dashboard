...
def test_forecast_builder_upload_validate_calculate_and_download(page: Page) -> None:
    ...
    expect(body).to_contain_text("Heavy RUC: exact stored prediction replay is available", timeout=90000)
    expect(body).to_contain_text("new-row scoring is unavailable until parent-state parity passes", timeout=90000)
    ...
    expect(body).to_contain_text("training-fit R2/provenance is available from source-refit state", timeout=60000)
    ...
    expect(body).to_contain_text("Historical actual", timeout=90000)
    expect(body).to_contain_text("Governed gap: Heavy new-row scoring unavailable until parent-state parity passes", timeout=90000)
    forecast_stream = page.get_by_label("Forecast stream")
    forecast_stream.click()
